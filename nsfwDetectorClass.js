class NsfwDetector {
    constructor() {
        ...
        this._nsfwSfwLabels = ['NSFW', 'SFW'];

        this._classifierPromise = window.tensorflowPipeline('image-classification', 'AdamCodd/vit-base-nsfw-detector');
        this._classifierPromise2 = window.tensorflowPipeline('zero-shot-image-classification', 'Xenova/clip-vit-base-patch32');
    }
    ...
    async isNsfw(imageUrl) {
        ...
        try {
            ...
            // First, use the AdamCodd/vit-base-nsfw-detector model
            const nsfwDetector = await this._classifierPromise;
            const nsfwOutput = await nsfwDetector(blobUrl, this._nsfwSfwLabels);
            const isNsfwFirstPass = nsfwOutput[0].label === 'NSFW';

            if (isNsfwFirstPass) {
                console.log(`Classification for ${imageUrl}:`, 'NSFW (First pass)');
                return true;
            }

            // If the first model classifies the image as SFW, run it through the second model
            const classifier = await this._classifierPromise2;
